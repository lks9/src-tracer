...
* Instrument it (the pre-processed version)
  ```
  mkdir function_database
  python ../instrumenter.py checksum_inst.c function_database
  ```
...
* Display the trace
  ```
  python ../print_trace.py checksum_inst.c.trace function_database
  ```
### Retracing
...
* Retrace it (use `python -i` to work with the traced `state` in the interactive shell)
  ```
  python ../replay_trace.py checksum_retrace main checksum_inst.c.trace function_database
  echo "F1TTN" > sub.trace.txt
  python ../replay_trace.py checksum_retrace checksum sub.trace.txt function_database
  ```
